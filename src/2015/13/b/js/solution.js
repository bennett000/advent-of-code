...
function accumulateDeltaHappiness(map, deltaHappiness, person, i, arr) {
  ...
  const personsHappiness = map[person][lh] || 0;
  const lhHappiness = map[lh][person] || 0;

  return deltaHappiness + personsHappiness + lhHappiness;
}
...
function createHappinessMap(str, map = {}) {
  return str
    .split('\n')
    .reduce(parseInstruction, map);
}
