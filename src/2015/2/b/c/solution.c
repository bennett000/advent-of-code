...
uint squareFootage(uint small, uint mid, uint lg);
...
int main() {
  ...
  while (atEof == FALSE) {
    squareFeet += computeInput(&squareFootage, buffs, &atEof);
    zeroBuffers(buffs);
  }

  mfree(buffs);

  mprintf("The elves require %d feet of ribbon\n", squareFeet);
...
uint squareFootage(uint small, uint mid, uint lg) {
  return /* wrap */ small * 2 + mid * 2 + /* bow */ small * mid * lg;
}
