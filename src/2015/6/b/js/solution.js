...
const on = partial(map, state => state + 1);
const off = partial(map, state => state <= 0 ? 0 : state - 1);
const toggle = partial(map, state => state + 2);
...
const inc = (state, val) => state + val;

const brightness = strings
  .reduce((count, stringFn) => {
    // this function mutates
    stringFn(grid);

    return grid.reduce((colCount, row) => colCount + row.reduce(inc, 0), 0);
  }, 0);


console.log('There would be a total brightness of', brightness);
